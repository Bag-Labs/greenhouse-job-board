jQuery(document).ready(function($) {
	...
	$('.greenhouse-job-board').on('click', '.job_apply', function(e){
		...
		//accordion
		if ( $('.greenhouse-job-board').data('type') === 'accordion' ) {
		...
		//accordion & inline
			if ( $('.greenhouse-job-board').data('form_type') === 'inline') {
				...
				if ( $(this).hasClass('open') ) {
					$(this).text( $(this).data('closed-text') );
					$(this).removeClass('open');
					if ( $('.apply_jobs').hasClass('open') ) {
						$('.apply_jobs').removeClass('open');
						$('.apply_jobs').toggle(false);
					}
				}
				else {
					$('.job_apply').removeClass('open');
					$('.job_apply').toggle(true);
					$('.apply_ty').toggle(false);
					$('.job_apply').text( $(this).data('closed-text') );
					if ( $('.apply_jobs').hasClass('open') ) {
						$('.apply_jobs').removeClass('open');
					}
					//open this one
					$(this).addClass('open');
					$('.apply_jobs').addClass('open');
					$('.apply_jobs').toggle(true);
					$(this).text( $(this).data('opened-text') );
					
					//move form to this job in the DOM
					$('.apply_jobs').insertAfter( $(this).parents('.job') );
		...
		//cycle
		else if ( $('.greenhouse-job-board').data('type') === 'cycle' ) {
			...
			//cycle & inline
			else if ( $('.greenhouse-job-board').data('form_type') === 'inline' ) {
	...
	function thanks_message(){
		...
		//accordion
		else if ( $('.greenhouse-job-board').data('type') === 'accordion' ) {
			//hide form, display ty message in place
			$('#apply_form').toggle(false);
			$('.apply_ty').toggle(true);
		}
	}
