...
class Greenhouse_Job_Board_Public {
	 ...
	 * @param  string $content String of contents.
	 */
	public function greenhouse_shortcode_function( $atts, $content = null ) {
		...
		$atts = shortcode_atts(
			array(
				...
				'enable_demographics' => isset( $options['greenhouse_job_board_enable_demographics'] ) ? $options['greenhouse_job_board_enable_demographics'] : 'false',
			), $atts
		);
		...
		if ( 'true' !== $atts['hide_forms'] ) {
			$atts['hide_forms'] = 'false';
		}
		// set form_type.
		if ( 'default' === $atts['form_type'] ) {
			$atts['form_type'] = '';
		}
...
';
		...
		if ( 'cycle' !== $atts['board_type'] ) { // accordian.
			$ghjb_html .= '><div class="jobs" ';
		} elseif ( 'cycle' === $atts['board_type'] ) { // cycle.
		...
		if ( '' !== $atts['group'] ) {
			...
			if ( 'false' === $atts['group_headline'] ) {
				// group headlines? true (default) or false.
		...
		// cycle.
		if ( 'true' !== $atts['hide_forms'] &&
			'cycle' === $atts['board_type']
		) {
			$ghjb_html .= '<div class="cycle-slide"><div class="apply_jobs">
					<h1>' . $options['greenhouse_job_board_apply_headline'] . '</h1>';
					// cycle iframe.
			if ( 'iframe' === $atts['form_type'] ) {
				wp_enqueue_script( 'ghjb' . $atts['url_token'], 'https://app.greenhouse.io/embed/job_board/js?for=' . $atts['url_token'] );
				$ghjb_html .= '<div id="grnhse_app"></div>';// script for loading iframe.
			} // cycle inline.
			elseif ( 'inline' === $atts['form_type'] ) {
				$ghjb_html .= '<form id="apply_form" method="POST" action="' . plugins_url( '/greenhouse-job-board/public/partials/greenhouse-job-board-apply-submit.php' ) . '" enctype="multipart/form-data" data-enable-demographics="' . $atts['enable_demographics'] .'">
						</form>';
			}
		...
		elseif ( 'true' !== $atts['hide_forms'] &&
			'inline' === $atts['form_type'] &&
			'accordion' === $atts['board_type']
		) {
		...
		$ghjb_json = get_transient( 'ghjb_json' );
		if ( false === $ghjb_json ) {
		...
		$ghjb_jobs = get_transient( 'ghjb_jobs' );
		if ( false === $ghjb_jobs ) {
			// It wasn't there, so regenerate the data and save the transient.
		...
		if ( 1 === $shortcode_id ) {
			...
			if ( isset( $options['greenhouse_job_board_analytics'] ) &&
				'true' === $options['greenhouse_job_board_analytics'] ) {
				$ghjb_html .= 'true';
			} else {
				$ghjb_html .= '0';
			}
	 ...
	 * @since   2.4.0
	 */
	public function greenhouse_job_board_add_admin_menu() {
	 ...
	 * @since   2.4.0
	 */
	public function greenhouse_job_board_settings_init() {
		...
		add_settings_field(
			'greenhouse_job_board_demographics',
			__( 'Enable Demographics', 'greenhouse_job_board' ),
			'greenhouse_job_board_demographics_render',
			'greenhouse_settings',
			'greenhouse_job_board_jobboard_settings_section'
		);
